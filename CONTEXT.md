...
- **Auth-Sensitive PWA Caching:** Do not cache navigated HTML pages or Next runtime chunks cache-first in the PWA service worker because stale user/session markup or stale `/_next/static` chunks can produce signed-in state leaks or React hydration mismatches after a redesign/deploy. Use network-first/no-cache navigation, unregister the service worker in local dev, and bump SW cache names when changing caching behavior.
...
- **Google Search Site Name:** Keep `SITE_NAME`, Open Graph `siteName`, `applicationName`, root/homepage `WebSite` JSON-LD, and homepage `WebApplication` JSON-LD aligned around "Perks Reminder" so Google has explicit site-name signals. Use "without connecting bank accounts" wording in descriptions instead of "bank access" phrasing that can read awkwardly in snippets.
- For public Perks Reminder positioning, lead with spreadsheet replacement plus open-source/no-bank-login trust, Benefit Usage Guides, duplicate-card tracking, import/export, and community corrections. Avoid catalog-size or household-tracking claims until implemented.
